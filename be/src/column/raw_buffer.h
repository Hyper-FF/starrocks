...
public:
    ...
    size_t allocated_bytes() const { return _end_of_storage == _start ? 0 : _end_of_storage - _start + kPadding; }
...
void RawBuffer<T, padding>::release(memory::Allocator* allocator) {
    ...
    destroy(begin(), end());
    allocator->free(reinterpret_cast<void*>(_start), _end_of_storage - _start + kPadding);
...
void RawBuffer<T, padding>::relocate(memory::Allocator* allocator, size_t new_allocated_bytes) {
    if (UNLIKELY(_end_of_storage == _start)) {
    ...
    if constexpr (is_relocatable_v<T>) {
        ...
        if (old_allocated_bytes >= 4096) {
            new_start = reinterpret_cast<uint8_t*>(
                    allocator->realloc(reinterpret_cast<void*>(_start), old_allocated_bytes, new_allocated_bytes));
        } else {
            new_start = reinterpret_cast<uint8_t*>(allocator->alloc(new_allocated_bytes));
...
void RawBuffer<T, padding>::grow(memory::Allocator* allocator) {
    if (UNLIKELY(_end_of_storage == _start)) {
        // First allocation: call allocator directly (avoids relocate function call overhead).
        size_t new_bytes = required_bytes(1);
        _start = reinterpret_cast<uint8_t*>(allocator->alloc(new_bytes));
        DCHECK(_start != nullptr);
        _end = _start;
        _end_of_storage = _start + new_bytes - kPadding;
    } else {
...
void RawBuffer<T, padding>::reserve(memory::Allocator* allocator, size_t new_cap) {
    ...
    // Fast path for first allocation. This is the hottest path for reserve-only workloads.
    if (UNLIKELY(_end_of_storage == _start)) {
        _start = reinterpret_cast<uint8_t*>(allocator->alloc(new_allocated_bytes));
        DCHECK(_start != nullptr);
        _end = _start;
...
void RawBuffer<T, padding>::push_back(memory::Allocator* allocator, T&& value) {
    ...
    if constexpr (std::is_trivially_copyable_v<T>) {
        *reinterpret_cast<T*>(_end) = std::move(value);
    } else {
