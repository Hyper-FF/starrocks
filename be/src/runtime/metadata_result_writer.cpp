...
Status MetadataResultWriter::_fill_iceberg_metadata(const Columns& columns, const Chunk* chunk,
    ...
    const auto* content = static_cast<const Int32Column*>(ColumnHelper::get_data_column(columns[0].get()));
    const auto* file_path = static_cast<const BinaryColumn*>(ColumnHelper::get_data_column(columns[1].get()));
    const auto* file_format = static_cast<const BinaryColumn*>(ColumnHelper::get_data_column(columns[2].get()));
    const auto* spec_id = static_cast<const Int32Column*>(ColumnHelper::get_data_column(columns[3].get()));
    const auto* partition_data = static_cast<const BinaryColumn*>(ColumnHelper::get_data_column(columns[4].get()));
    const auto* record_count = static_cast<const Int64Column*>(ColumnHelper::get_data_column(columns[5].get()));
    const auto* file_size_in_bytes = static_cast<const Int64Column*>(ColumnHelper::get_data_column(columns[6].get()));
    const auto* split_offsets = static_cast<const ArrayColumn*>(ColumnHelper::get_data_column(columns[7].get()));
