...
Status DefaultColumnReader::get_next(orc::ColumnVectorBatch* cvb, Column* col, size_t from, size_t size) {
...
Status IntColumnReader<Type>::get_next(orc::ColumnVectorBatch* cvb, Column* col, size_t from, size_t size) {
    if (_nullable) {
        {
...
Status IntColumnReader<Type>::_fill_int_column_with_null_from_cvb(OrcColumnVectorBatch* data, Column* col, size_t from,
                                                                  size_t size) {
    ...
    auto* values = ColumnHelper::cast_to_raw<Type>(null_column->data_column_raw_ptr())->get_data().data();
                ...
                if (nulls[i] == 0 && (value < numeric_limits<RunTimeCppType<Type>>::lowest() ||
                    ...
                    if (!reported) {
                        ...
                        std::string error_msg =
                                absl::Substitute("Value '$0' is out of range. The type of '$1' is $2'",
                                                 std::to_string(value), slot->col_name(), slot->type().debug_string());
...
template <LogicalType Type>
template <typename OrcColumnVectorBatch>
Status IntColumnReader<Type>::_fill_int_column_from_cvb(OrcColumnVectorBatch* data, Column* col, size_t from,
                                                        size_t size) {
    size_t col_start = col->size();
    col->resize_uninitialized(col_start + size);

    auto* values = ColumnHelper::cast_to_raw<Type>(col);
    auto* mutable_values = const_cast<RunTimeColumnType<Type>*>(values);
    auto* data_ptr = mutable_values->get_data().data();

    auto* cvbd = data->data.data();

    for (size_t i = col_start, cvb_pos = from; i < col_start + size; ++i, ++cvb_pos) {
        data_ptr[i] = cvbd[cvb_pos];
    }
...
template <LogicalType Type>
Status DoubleColumnReader<Type>::get_next(orc::ColumnVectorBatch* cvb, Column* column, size_t from, size_t size) {
    size_t column_start = column->size();
    column->resize_uninitialized(column_start + size);
    if (_nullable) {
        auto null_column = ColumnHelper::as_raw_column<NullableColumn>(column);
...
template <LogicalType Type>
template <typename OrcColumnVectorBatch>
Status DoubleColumnReader<Type>::_fill_double_column_from_cvb(OrcColumnVectorBatch* data, Column* col,
...
Status DecimalColumnReader::get_next(orc::ColumnVectorBatch* cvb, Column* col, size_t from, size_t size) {
    size_t col_start = col->size();
    col->resize_uninitialized(col_start + size);
...
Status Decimal32Or64Or128ColumnReader<DecimalType>::get_next(orc::ColumnVectorBatch* cvb, Column* col, size_t from,
                                                             size_t size) {
    if (dynamic_cast<orc::Decimal64VectorBatch*>(cvb) != nullptr) {
...
inline void Decimal32Or64Or128ColumnReader<DecimalType>::_fill_decimal_column_generic(orc::ColumnVectorBatch* cvb,
    ...
    if (_nullable) {
        ...
        decimal_column =
                const_cast<ColumnType*>(ColumnHelper::cast_to_raw<DecimalType>(nullable_column->data_column_raw_ptr()));
    } else {
        decimal_column = const_cast<RunTimeColumnType<DecimalType>*>(ColumnHelper::cast_to_raw<DecimalType>(col));
    }
...
Status StringColumnReader::get_next(orc::ColumnVectorBatch* cvb, Column* col, size_t from, size_t size) {
    ...
    Column* data_column = nullptr;
    if (_nullable) {
        auto* c = ColumnHelper::as_raw_column<NullableColumn>(col);
        c->null_column_raw_ptr()->resize_uninitialized(col->size() + size);
        handle_null(cvb, c, col_start, from, size, false);
        data_column = c->data_column_raw_ptr();
    } else {
        data_column = col;
    }

    auto* values = ColumnHelper::cast_to_raw<TYPE_VARCHAR>(data_column);
...
Status VarbinaryColumnReader::get_next(orc::ColumnVectorBatch* cvb, Column* col, size_t from, size_t size) {
    ...
    auto* values = ColumnHelper::cast_to_raw<TYPE_VARBINARY>(data_column);
    auto& vb = values->get_bytes();
    auto& vo = values->get_offset();
...
Status MapColumnReader::_fill_map_column(orc::ColumnVectorBatch* cvb, Column* col, size_t from, size_t size) {
    ...
    auto* values = col_map->values_column_raw_ptr();
...
Status StructColumnReader::_fill_struct_column(orc::ColumnVectorBatch* cvb, Column* col, size_t from, size_t size) {
    ...
    for (size_t i = 0; i < _type.children.size(); i++) {
        ...
        auto* field_column = field_columns[i]->as_mutable_raw_ptr();
        RETURN_IF_ERROR(_child_readers[i]->get_next(field_cvb, field_column, from, size));
    }
    return Status::OK();
}
