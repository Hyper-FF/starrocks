...
#define ASSIGN_BOX_TYPE(NAME, TYPE)                                                \
    ...
    case TYPE_DECIMAL32:
    case TYPE_DECIMAL64: {
        // DECIMAL32/64: ask Java for the unscaled value as a long; handle overflow by
        // catching the ArithmeticException UDFHelper.unscaledLong throws.
        auto* env = helper.getEnv();
        jlong unscaled = helper.unscaled_long(val.l, type_desc.precision, type_desc.scale);
        RETURN_IF_ERROR(handle_decimal_overflow(env, type_desc, col, row_num, error_if_overflow));
        if (env->ExceptionCheck()) return Status::OK(); // cleared & nulled by helper
    ...
    case TYPE_DECIMAL256: {
        ...
        // the int128/int256 cell at row_num.
        auto* env = helper.getEnv();
