...
#include "udf/java/jni_arrow_func_call_stub.h"
...
#include "udf/java/arrow_udf_jni.h"
...
std::unique_ptr<UDFCallStub> create_jni_arrow_call_stub(FunctionContext* ctx, RuntimeState* state,
    ...
    auto build = [&]() -> Status {
        ...
        ASSIGN_OR_RETURN(auto evaluate_obj, jni_ctx->analyzer->get_method_object(jni_ctx->udf_class.clazz(), "evaluate"));
        ...
        RETURN_ERROR_IF_JNI_EXCEPTION(env);
        jni_ctx->evaluate_arrow_mid = env->GetStaticMethodID(
                helper, "evaluateArrow", "(Ljava/lang/Object;Ljava/lang/reflect/Method;JJJJJ)V");
        RETURN_ERROR_IF_JNI_EXCEPTION(env);
