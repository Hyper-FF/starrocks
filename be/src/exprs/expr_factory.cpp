...
Status create_vectorized_expr(ObjectPool* pool, const TExprNode& texpr_node, Expr** expr, RuntimeState* state) {
    ...
    case TExprNodeType::NULL_LITERAL: {
        *expr = pool_alloc<VectorizedLiteral>(pool, mp, texpr_node);
        break;
    }
    ...
    case TExprNodeType::SLOT_REF: {
        ...
        *expr = pool_alloc<ColumnRef>(pool, mp, texpr_node);
        break;
    }
    ...
    case TExprNodeType::INFO_FUNC:
        *expr = pool_alloc<VectorizedInfoFunc>(pool, mp, texpr_node);
        break;
    case TExprNodeType::PLACEHOLDER_EXPR:
        *expr = pool_alloc<PlaceHolderRef>(pool, mp, texpr_node);
        break;
    case TExprNodeType::DICT_EXPR:
        RETURN_IF_ERROR(try_non_core_create_post_hook(pool, texpr_node, expr, state));
        break;
    case TExprNodeType::LAMBDA_FUNCTION_EXPR:
        *expr = pool_alloc<LambdaFunction>(pool, mp, texpr_node);
        break;
    case TExprNodeType::CLONE_EXPR:
        *expr = pool_alloc<CloneExpr>(pool, mp, texpr_node);
        break;
    ...
    case TExprNodeType::MATCH_EXPR:
        *expr = pool_alloc<MatchExpr>(pool, mp, texpr_node);
        break;
...
Status ExprFactory::create_expr_tree(ObjectPool* pool, const TExpr& texpr, ExprContext** ctx, RuntimeState* state,
    ...
    RETURN_IF_ERROR(create_expr_tree(pool, texpr, &root_expr, state, can_jit));
    MemPool* mp = fragment_mem_pool_of(state, pool);
    *ctx = root_expr == nullptr ? nullptr : pool_alloc<ExprContext>(pool, mp, root_expr);
    return Status::OK();
}
...
Status ExprFactory::create_expr_trees(ObjectPool* pool, const std::vector<TExpr>& texprs,
    ...
    for (Expr* root_expr : root_exprs) {
        ctxs->push_back(root_expr == nullptr ? nullptr : pool_alloc<ExprContext>(pool, mp, root_expr));
    }
    return Status::OK();
}
