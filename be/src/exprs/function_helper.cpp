...
void FunctionHelper::union_produce_nullable_column(const ColumnPtr& v1, const ColumnPtr& v2,
    ...
    if (v1->has_null()) {
        auto* null1 = static_cast<const NullableColumn*>(v1.get())->null_column()->immutable_data().data();
    ...
    if (v2->has_null()) {
        auto* null2 = static_cast<const NullableColumn*>(v2.get())->null_column()->immutable_data().data();
...
void FunctionHelper::union_produce_nullable_column(const ColumnPtr& v1, NullColumn::MutablePtr* produce_null_column) {
    ...
    if (v1->has_null()) {
        const auto* null1 = static_cast<const NullableColumn*>(v1.get())->null_column()->immutable_data().data();
...
ColumnPtr FunctionHelper::merge_column_and_null_column(ColumnPtr&& column, NullColumnPtr&& null_column) {
    if (column->only_null()) {
        ...
        const auto* const_column = static_cast<const ConstColumn*>(column.get());
