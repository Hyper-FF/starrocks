...
public:
    ...
    void update(FunctionContext* ctx, const Column** columns, AggDataPtr state, size_t row_num) const override {
        const auto* column = static_cast<const PercentileColumn*>(columns[0]);
    ...
    void merge(FunctionContext* ctx, const Column* column, AggDataPtr __restrict state, size_t row_num) const override {
        ...
        const auto* percentile_column = static_cast<const PercentileColumn*>(column);
        update_state(ctx, state, percentile_column->get_object(row_num));
    }

    void serialize_to_column(FunctionContext* ctx, ConstAggDataPtr __restrict state, Column* to) const override {
        DCHECK(to->is_object());
        auto* column = static_cast<PercentileColumn*>(to);
