...
Status PartitionedSpillerWriter::_compact_skew_chunks(size_t num_rows, std::vector<ChunkPtr>& chunks,
    ...
    for (auto idx : samples) {
        ...
        auto* hash_column = static_cast<const UInt32Column*>(curr_chunk->columns().back().get());
    ...
    for (auto& chunk : chunks) {
        ...
        auto* hash_column = static_cast<const UInt32Column*>(chunk->columns().back().get());
        auto& hash_values = hash_column->get_data();
        ...
        while (true) {
            {
                ...
                auto hash_column = static_cast<const SpillHashColumn*>(chunk->columns().back().get());
