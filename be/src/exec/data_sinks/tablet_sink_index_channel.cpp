...
Status NodeChannel::_send_request(bool eos, bool finished) {
    ...
    // `request` was constructed inside the SCOPED(nullptr) mem-tracker scope and is a
    // local variable; it is destroyed before SCOPED ends, so all its protobuf buffers
    // are freed under nullptr (process) — no Swap trick required.
    return Status::OK();
}
