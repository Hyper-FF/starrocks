...
StatusOr<pipeline::OpFactories> HashJoinNode::_decompose_to_pipeline(pipeline::PipelineBuilderContext* context) {
    ...
    const size_t probe_pipelines_begin = context->num_pipelines();
    ...
    const bool align_both_sides = !join_in_colocate_group &&
                                  _distribution_mode != TJoinDistributionMode::BROADCAST &&
                                  context->could_local_shuffle(rhs_operators) !=
                                          context->could_local_shuffle(lhs_operators);
    ...
    // push_dependent_pipeline would have given them.
    context->subscribe_pipelines_since(probe_pipelines_begin, build_pipeline,
