...
void TableBuilder::Flush() {
    ...
    if (ok()) {
        ...
        // do nothing here, sync when finish
...
static bool Snappy_Compress(const char* input, size_t length, ::std::string* output) {
...
void TableBuilder::WriteRawBlock(const Slice& block_contents, CompressionType type, BlockHandle* handle) {
    ...
    handle->set_size(block_contents.get_size());
    r->status = r->file->append(block_contents);
    if (r->status.ok()) {
        char trailer[kBlockTrailerSize];
        trailer[0] = type;
        uint32_t crc = crc32c::Value(block_contents.get_data(), block_contents.get_size());
        crc = crc32c::Extend(crc, trailer, 1); // Extend crc to cover block type
        EncodeFixed32(trailer + 1, crc32c::Mask(crc));
        r->status = r->file->append(Slice(trailer, kBlockTrailerSize));
        if (r->status.ok()) {
            r->offset += block_contents.get_size() + kBlockTrailerSize;
        }
    }
}
