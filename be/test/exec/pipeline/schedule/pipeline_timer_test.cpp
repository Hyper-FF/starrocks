...
protected:
    void SetUp() override { ASSERT_OK(_timer.start()); }
...
TEST_F(PipelineTimerTaskTest, runs_when_due_and_unschedule_after_done_does_not_block) {
    ProbeTask task;
    ASSERT_OK(_timer.schedule(&task, past_abstime()));
...
TEST_F(PipelineTimerTaskTest, unschedule_pending_task_removes_without_wait) {
    ProbeTask task;
    ASSERT_OK(_timer.schedule(&task, future_abstime(3600)));

    int rc = _timer.unschedule(&task);
...
TEST_F(PipelineTimerTaskTest, batched_tasks_all_unblock_eventually) {
    ...
    for (auto& t : tasks) {
        ASSERT_OK(_timer.schedule(t.get(), when));
    }
...
TEST_F(PipelineTimerTaskTest, light_timer_task_runs_and_unschedules) {
    {
        LightProbe expired;
        ASSERT_OK(_timer.schedule(&expired, past_abstime()));
    ...
    {
        LightProbe pending;
        ASSERT_OK(_timer.schedule(&pending, future_abstime(3600)));
        int rc = _timer.unschedule(&pending);
        EXPECT_EQ(rc, TIMER_TASK_REMOVED);
        std::this_thread::sleep_for(std::chrono::milliseconds(50));
