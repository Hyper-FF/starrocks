...
TEST(VariantRowConverterTest, CastArithmeticAndTemporalValues) {
    ...
    Status int_status =
            VariantRowConverter::cast_to<TYPE_BIGINT, true>(int_row.as_ref(), cctz::utc_time_zone(), int_builder);
    ...
    Status date_status =
            VariantRowConverter::cast_to<TYPE_DATE, true>(date_row.as_ref(), cctz::utc_time_zone(), date_builder);
    ...
    Status time_status =
            VariantRowConverter::cast_to<TYPE_TIME, true>(time_row.as_ref(), cctz::utc_time_zone(), time_builder);
    ...
    Status datetime_status = VariantRowConverter::cast_to<TYPE_DATETIME, true>(datetime_row.as_ref(),
...
TEST(VariantRowConverterTest, CastBooleanAndString) {
    VariantRowValue bool_row = make_variant_row(encode_string("TrUe"));
    ColumnBuilder<TYPE_BOOLEAN> bool_builder(1);
    Status bool_status =
            VariantRowConverter::cast_to<TYPE_BOOLEAN, true>(bool_row.as_ref(), cctz::utc_time_zone(), bool_builder);
    ...
    Status string_status =
            VariantRowConverter::cast_to<TYPE_VARCHAR, true>(int_row.as_ref(), cctz::utc_time_zone(), string_builder);
...
TEST(VariantRowConverterTest, ThrowAndNullOnInvalidBooleanString) {
    VariantRowValue invalid_bool_row = make_variant_row(encode_string("not_bool"));

    ColumnBuilder<TYPE_BOOLEAN> throw_builder(1);
    auto throw_status = VariantRowConverter::cast_to<TYPE_BOOLEAN, true>(invalid_bool_row.as_ref(),
                                                                         cctz::utc_time_zone(), throw_builder);
    EXPECT_FALSE(throw_status.ok());

    ColumnBuilder<TYPE_BOOLEAN> null_builder(1);
    Status null_status = VariantRowConverter::cast_to<TYPE_BOOLEAN, false>(invalid_bool_row.as_ref(),
...
TEST(VariantRowConverterTest, CastFromRowRefMatchesRowValue) {
    ...
    Status from_value_status = VariantRowConverter::cast_to<TYPE_BIGINT, true>(int_row.as_ref(), cctz::utc_time_zone(),
                                                                               from_value_builder);
    ...
    Status from_ref_status =
            VariantRowConverter::cast_to<TYPE_BIGINT, true>(int_ref, cctz::utc_time_zone(), from_ref_builder);
