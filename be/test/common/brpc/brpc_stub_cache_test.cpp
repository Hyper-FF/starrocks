...
// because the task runs on the guarded timer path (BthreadTimerTask::doRun() holds
// `auto self = shared_from_this()`); on an unguarded firing path the reschedule would free `this`
// mid-Run() -> heap-use-after-free (ASan aborts on the timer thread).
//
...
TEST_F(BrpcStubCacheTest, cleanup_task_reschedule_no_use_after_free) {
    ...
    cache._timer->unschedule(task);
    timespec now_ts = butil::microseconds_to_timespec(butil::gettimeofday_us());
    ASSERT_OK(cache._timer->schedule(task, now_ts));
