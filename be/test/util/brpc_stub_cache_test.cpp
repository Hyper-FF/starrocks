...
// Deterministic regression test for the cleanup-task use-after-free fixed in #76931.
//
...
// reference, and then keeps reading its own members (_deadline/_cache/_endpoint). On the unguarded
// LightTimerTask firing path (RunLightTimerTask -> Run() with no owning reference held) that frees
// `this` mid-Run() -> heap-use-after-free (ASan aborts on the brpc_timer thread). The fix runs the
// task on the guarded PipelineTimerTask path (RunTimerTask -> doRun() holds
// `auto self = shared_from_this()`), so the task stays alive through Run().
//
...
// future, so the fire takes the reschedule branch rather than the evict branch. With the fix this
// completes cleanly; reverting the fix (base class back to LightTimerTask) makes this test abort
// under ASan.
