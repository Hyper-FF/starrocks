...
public class TimeSliceRewriteEquivalent extends IPredicateRewriteEquivalent {
    ...
    // every row in [c, c+interval)), so the rewrite must be declined for them. This is stricter than
    // DateTruncEquivalent (which also allows GT) because time_slice always buckets.
    ...
    public ScalarOperator rewrite(RewriteEquivalentContext eqContext,
        ...
        if (!isSupportedBinaryType(((BinaryPredicateOperator) newInput).getBinaryType())) {
            return null;
        }
