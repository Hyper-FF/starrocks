...
public final class ExprNegateFunction {
     ...
     *
     * <p>{@link #isSupportNegate} is consulted here and nowhere else, so whatever it declines is
     * wrapped rather than inverted. Keeping the decision in one place is what stops the two from
     * drifting apart: {@code ExprUtils#pushNegationToOperands} checked {@code isSupportNegate} on
     * the NOT's direct child only, and {@link #negateCompoundPredicate} then recursed with a bare
     * {@code negate()} on each operand. So {@code NOT (a <=> b)} was correctly left alone --
     * {@code EQ_FOR_NULL} is not in the supported list -- while {@code NOT (a <=> b AND c)} passed
     * the outer check as a compound and reached {@link #negateBinaryPredicate}, whose switch has no
     * case for it, throwing {@code IllegalStateException} out of the analyzer on valid SQL.
     *
     * <p>De Morgan still holds with the wrapper: {@code NOT (a <=> b AND c)} becomes
     * {@code NOT(a <=> b) OR NOT(c)}, correct if less flat than a fully inverted tree.
     */
    ...
    private static Expr negateBinaryPredicate(BinaryPredicate predicate) {
        ...
        switch (predicate.getOp()) {
            ...
            case GT:
                ...
                break;
            default:
                // Unreachable through negate(), which gates on isSupportNegate first. A fallback
                // rather than a throw so that a BinaryType added to one list and not the other
                // costs an un-flattened NOT wrapper instead of an internal error on user SQL --
                // which is what EQ_FOR_NULL cost. A new operator belongs in isSupportNegate AND in
                // this switch; getting only the first right is now merely suboptimal.
